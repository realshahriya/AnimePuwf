...
import { fetchByHandle } from "@/lib/supabase";
import Link from "next/link";

type ResultState = CardResult;

export default function PuwfResultPage({ params }: { params: Promise<{ universe: string, handle?: string[] }> }) {
  ...
  const publicHandle = handle && handle.length > 0 ? handle[0] : null;

  useEffect(() => {
    const fetchPublicResult = async () => {
      const supabase = createClient();
      const row = await fetchByHandle(supabase, publicHandle!, universeSlug);
      if (row) {
        setResult({
        ...
        // Handle 404 or missing basically just by not setting result (shows loading eternally or we can add error state)
      }
    };
  ...
  const cleanHandle = result.handle?.replace(/^@/, "");
  const pageUrl = cleanHandle 
    ? `${typeof window !== "undefined" ? window.location.origin : "https://animepuwf.com"}/${universeSlug}/puwf/result/${cleanHandle}`
    : typeof window !== "undefined" ? window.location.href : `https://animepuwf.com/${universeSlug}/puwf`;
    ...
    <main className="w-full max-w-5xl mx-auto px-4 md:px-6 pt-28 md:pt-32 pb-12 text-light-ash flex flex-col items-center min-h-[80vh]">
      ...
      >
        ...
        <p className="text-light-ash/40 max-w-lg mx-auto text-sm leading-relaxed">
          {publicHandle 
            ? `The identity of ${result.userName} within the ${universe?.name} universe has been extracted and encoded into this digital relic.`
      ...
      {/* Challenge Call To Action button for public profiles */}
      {publicHandle && (
        <motion.div 
        ...
        >
          <Link href={`/${universeSlug}/puwf/${cleanHandle}`}>
             <button className="w-full py-4 text-center rounded-xl bg-puwf-fire text-white font-bold tracking-widest uppercase transition-all shadow-[0_0_20px_rgba(255,107,0,0.4)] hover:shadow-[0_0_30px_rgba(255,107,0,0.6)] hover:-translate-y-1 block">
               Find Your Power & Challenge
             </button>
          </Link>
        </motion.div>
      )}
...
151:           </div>
152: 
153:           {/* Secondary row — native share (mobile) + copy link */}
154:           <div className="flex gap-3">
155:             {canNativeShare && (
156:               <button
157:                 id="share-native"
158:                 onClick={handleNativeShare}
159:                 className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-puwf-fire/10 hover:bg-puwf-fire/20 border border-puwf-fire/20 rounded-lg text-xs font-bold tracking-widest uppercase transition-all text-puwf-fire"
160:               >
161:                 <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" /></svg>
162:                 Share
163:               </button>
164:             )}
165:             <button
166:               id="share-copy"
167:               onClick={handleCopyLink}
168:               className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 border rounded-lg text-xs font-bold tracking-widest uppercase transition-all ${
169:                 copied
170:                   ? "bg-green-500/20 border-green-500/40 text-green-400"
171:                   : "bg-white/5 hover:bg-white/10 border-white/5 text-light-ash"
172:               }`}
173:             >
174:               {copied ? (
175:                 <>
176:                   <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
177:                   Copied!
178:                 </>
179:               ) : (
180:                 <>
181:                   <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
182:                   Copy Link
183:                 </>
184:               )}
185:             </button>
